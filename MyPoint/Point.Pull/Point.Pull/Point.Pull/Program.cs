...
namespace Point.Pull
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            p.Pull_taobao_tbk_dg_item_coupon_get();
        }
        ...
        /// </summary>
        public void Pull_taobao_tbk_dg_item_coupon_get()
        {
            try
            {
                Console.WriteLine("准备开始拉取 好券清单API【导购】......");
                ...
                Console.WriteLine("结束拉取 好券清单API【导购】......");
                Console.WriteLine();
            }
            catch (Exception ex)
            {
        ...
        /// http://open.taobao.com/docs/api.htm?spm=a219a.7629065.0.0.7LescP&apiId=29821
        /// </summary>
        public void Pull_taobao_tbk_dg_item_coupon_get_bak()
        {
            try
            {
                Console.WriteLine("准备开始拉取 好券清单API【导购】......");
                Console.WriteLine("请输入查询关键字，回车之后开始拉取：");
                ...
                for (int i = 1; i <= 10; i++)
                {
                    Console.WriteLine(string.Format("开始第 {0} 次拉取...", i));
                    ...
                    if (response != null && response.Results != null && response.Results.Count > 0)
                    {
                        ...
                        Console.WriteLine("防止API频率限制，休眠 100 ms ....");
                        Thread.Sleep(100);
                        Console.WriteLine(string.Format("结束第 {0} 次拉取...成功拉取 {1} 条数据", i, response.Results.Count));
                    }
                    else
                    {
                        Console.WriteLine(" API 未能返回数据，退出本次拉取...");
