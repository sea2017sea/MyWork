...
(
SysNo INT IDENTITY(1,1) PRIMARY KEY,                    --主键，自增长，唯一
TranType INT NOT NULL,                                  --交易类型  1 回答问题（参与互动）    2 邀请好友（分享好友） 
                                                        --4 转出积分  8 转入积分 16 现金提现 32 兑换商品(兑换抵扣劵)   64 阅读文章付费
                                                        --70 新版参与互动  
...
DataType int NOT NULL,                --数据类型   1 广告（参与互动，领取红包）   2 推广(邀请好友，立享钜惠)  3 热卖推荐
...
CashBonus DECIMAL(18,4) NULL,                                            --现金红包 客户参与之后发给客户的红包金额
CashBonusNum int NULL,                                                   --现金红包的总数量
SalesVolume int NULL,                                                    --销量 对应 B_InforConfigure 表的 DataType = 3 热卖推荐的销售数量
IntSort int NOT NULL,                                                    --排序，数值越大越靠前
RowCeateDate DATETIME NOT NULL,                                          --创建时间
ModifyTime DATETIME NULL,                                                --修改时间
IsEnable BIT NOT NULL                                                    --是否启用 true 启用 0 禁用
...
SysNo INT IDENTITY(1,1) PRIMARY KEY,                                     --主键，自增长
UserId INT NOT NULL,                                                     --会员ID
CouponSysNo INT NOT NULL,                                                --优惠劵活动ID 对应 B_InforConfigure 表的 SysNo 主键
CashBonus DECIMAL(18,4) NULL,                                            --获取的现金红包 客户参与之后发给客户的红包金额
RowCeateDate DATETIME NOT NULL,                                          --创建时间
ModifyTime DATETIME NULL,                                                --修改时间
IsEnable BIT NOT NULL                                                    --是否启用 true 启用 0 禁用
...
--insert into B_AdvGoods values(1,1,'商品名称','商品图片地址','商品明细连接地址','商品兑换链接地址',123,343,23,23,232,12,1,'2018-07-05 14:17:42.000','2018-07-05 14:17:42.000',1)
